...
use dmm::{Coord, Map, MapFormat, Prefab, Size, Tile};
use editor::{
    EditorState,
    Environment,
...
use crate::{
    ...
    git_worker::{GitWorker, Outcome as GitOutcome, Revision},
...
impl BlameState {
    ...
    fn heatmap(&self, z: u32) -> SharedHighlights {
        self.heatmap
            .borrow_mut()
            .entry(z)
    ...
    fn outline(&self, z: u32, commit: u32) -> SharedHighlights {
        self.outlines
            .borrow_mut()
            .entry((z, commit))
            .or_insert_with(|| {
...
impl DiffSide {
    fn new(source: DiffSource, revision: Option<Revision>) -> Self {
...
impl DiffState {
    fn new(from: DiffSide, to: DiffSide) -> Self {
        Self {
            from,
            to,
...
impl Session {
    ...
    fn with_diff_view<R>(&self, id: DocumentId, read: impl FnOnce(&mut DiffView) -> R) -> Option<R> {
...
pub(crate) mod tests {
    ...
    use editor::{
        ...
        blame::BlameResult,
        command::EditGroupId,
        diff::{ChangeKind, DiffSource, MODIFIED_COLOR},
