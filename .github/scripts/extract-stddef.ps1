...
# Keep the repository's existing headerless representation of stddef.dm.
$body = $generated.Substring($bodyStart)
$body = $body.Replace("`r`n", "`n").Replace("`r", "`n")
$body = $body.TrimEnd([char[]] "`r`n") + "`n"

if ($body.Length -lt 1000 -or
    -not $body.Contains('#define NORTH 1') -or
    -not $body.Contains('var/const')) {
    throw 'The generated stddef.dm body failed its content checks.'
}

$OutputPath = [IO.Path]::GetFullPath($OutputPath)
[IO.File]::WriteAllText($OutputPath, $body, (New-Object Text.UTF8Encoding($false)))
